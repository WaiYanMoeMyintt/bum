...
import DropdownMenuDemo from "./UpdateTask";
      ...
      <div className="mt-4 flex flex-wrap w-full px-1  gap-2 ">
        {today &&
          today.map((items, index) => (
            <div className="tasks_content w-80 h-22 my-1 py-2 cursor-pointer px-4 flex gap-4 items-center justify-center rounded-xl">
              <Checkbox />
              <div className="w-full">
                <div className="flex justify-between items-center w-full">
                  <Link
                    href={`/today/${items?._id}`}
                    className="text-[#352e5d] mb-0.5 font-medium"
                  >
                    {items?.title}
                  </Link>
